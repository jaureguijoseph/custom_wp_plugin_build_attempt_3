Bricks Page Builder Theme In Line Files and code
...
\* Pre 1.2: \'\_bricks_page\_{\$content_type}\'
...
\* Order matters: \'cmb2\' before \'wp\' so it can filter the custom
fields correctly.
...
\$title = \$context ? sprintf( esc_html\_\_( \'Author: %s\', \'bricks\'
), \$author ) : \$author;
...
\$title = esc_html\_\_( \'Audio\', \'bricks\' );
...
\'archive-author\' =\> esc_html\_\_( \'Archive (author)\', \'bricks\' ),
...
// When editing with Elementor we need to tell Bricks to render
...
// Set to false if \'pauseOnHover\' is true to prevent swiper stopping
after first hover
...
return \[ \'error\' =\> esc_html\_\_( \'Code execution is disabled\',
\'bricks\' ) \];
...
// STEP: Code element: User doesn\'t have permission and code execution
...
\* Previously used get_post_type( \$post_id ) ===
...
\$set_aria_current = untrailingslashit( home_url( \'/\' ) ) ===
...
\$set_aria_current = self::is_post_ancestor( get_queried_object_id(),
\$post_id_of_link );
...
\$template_preview_term_id = isset(
\$template_preview_term_id_parts\[1\] ) ?
\$template_preview_term_id_parts\[1\] : \'\';
...
\$template_preview_search_term = self::get_template_setting(
\'templatePreviewSearchTerm\', \$post_id );
...
case \'single\': // (template condition \"Content type\" = \"Single
post/page\")
...
\$query_args = apply_filters( \'bricks/element/builder_setup_query\',
\$query_args, \$post_id );
...
\* Currently used in related-posts element and query_results_count when
targeting related-posts element
...
public static function populate_query_vars_for_element( \$element_data,
\$post_id ) {
...
\'bricks_skip_query_vars\' =\> true, // skip
Query::prepare_query_vars_from_settings() \@since 1.9.3
...
\$terms_ids = wp_get_post_terms( \$post_id, \$taxonomy, \[ \'fields\'
=\> \'ids\' \] );
...
// NOTE: Undocumented (not ideal naming as it\'s not only for
related-posts element)

\$args = apply_filters( \'bricks/related_posts/query_vars\', \$args,
\$settings );
...
// STEP: Media type carousel might use dynamic data, so we need to
normalize the settings (logic originally inside carousel element)

\$gallery_class_name =
Elements::\$elements\[\'image-gallery\'\]\[\'class\'\] ?? false;

if ( \$gallery_class_name && ! empty(
\$settings\[\'items\'\]\[\'useDynamicData\'\] ) ) {
...
// STEP: Populate query vars based on the \'items\' settings, if no
items are set, no query vars are populated
...
\$args\[\'post\_\_in\'\]\[\] = \$image\[\'id\'\];

}

}

if ( isset( \$args\[\'post\_\_in\'\] ) ) {

\$args\[\'posts_per_page\'\] = count( \$args\[\'post\_\_in\'\] );
...
return self::safe_implode( \$sep, \$item ); // Recursively flattern the
array value
...
\* Builds a hierarchical tree structure from a flat array of Bricks
elements.

\*

\* The tree structure is used to process each element in a depth-first
manner to maintain the hierarchy.

\*

\* Each element in the input array should be an associative array with
the following structure:

\* - id (string): Unique identifier for the element.

\* - name (string): The name/type of the element (e.g., \'section\',
\'container\', \'heading\').

\* - parent (string\|int\|null): The ID of the parent element, or 0/null
if it is a root element.

\* - children (array): An array of child element IDs (strings). Defaults
to an empty array.

\* - settings (array): An associative array of settings specific to the
...
self::maybe_log( \'Bricks: Invalid or empty elements array provided to
build_elements_tree\' );
...
if ( ! isset( \$element\[\'id\'\] ) \|\| ! is_string(
\$element\[\'id\'\] ) ) {

self::maybe_log( \'Bricks: Invalid element encountered: missing or
invalid ID\' );
...
if ( isset( \$element\[\'children\'\] ) && is_array(
\$element\[\'children\'\] ) ) {

\$children_order\[ \$element\[\'id\'\] \] = array_values( array_filter(
\$element\[\'children\'\], \'is_string\' ) );
...
if ( ! empty( \$element\[\'parent\'\] ) && isset( \$indexed\[
\$element\[\'parent\'\] \] ) ) {

// Add as child to parent

\$indexed\[ \$element\[\'parent\'\] \]\[\'children\'\]\[\] =
&\$indexed\[ \$element\[\'id\'\] \];
...
\* \@param array \$children_order Original order of children for each
element.
...
private static function sort_elements_tree_children( &\$elements,
\$children_order ) {
...
if ( ! empty( \$element\[\'children\'\] ) && isset( \$children_order\[
\$element\[\'id\'\] \] ) ) {
...
self::maybe_log( \'Bricks: Invalid child element encountered during
sorting\' );
...
\$pos_a = isset( \$order\[ \$a\[\'id\'\] \] ) ? \$order\[ \$a\[\'id\'\]
\] : PHP_INT_MAX;

\$pos_b = isset( \$order\[ \$b\[\'id\'\] \] ) ? \$order\[ \$b\[\'id\'\]
\] : PHP_INT_MAX;
...
self::sort_elements_tree_children( \$element\[\'children\'\],
\$children_order );
...
public static function get_html_tag_from_element_settings( \$settings,
\$default_tag ) {
...
if ( ! is_array( \$allowed_html_tags ) \|\| ! in_array( \$tag,
\$allowed_html_tags, true ) ) {
...
\$extended_allowed_html_tags = apply_filters(
\'bricks/allowed_html_tags\', \$allowed_html_tags );

// Merge default and extended HTML tags

\$allowed_html_tags = is_array( \$extended_allowed_html_tags ) ?
array_merge( \$allowed_html_tags, \$extended_allowed_html_tags ) :
\$allowed_html_tags;
...
// Store the original post before looping to restore the context (nested
loops)
...
public function \_\_construct( \$element = \[\] ) {

\$this-\>register_query();

\$this-\>element_id = ! empty( \$element\[\'id\'\] ) ?
\$element\[\'id\'\] : \'\';
...
// Assign the history query instance properties to this instance, avoid
...
\$element\[\'settings\'\]\[\'query\'\]\[\'objectType\'\] ) ?
\$element\[\'settings\'\]\[\'query\'\]\[\'objectType\'\] : \'post\';

// Remove object type from query vars to avoid future conflicts

unset( \$element\[\'settings\'\]\[\'query\'\]\[\'objectType\'\] );

\$this-\>settings = ! empty( \$element\[\'settings\'\] ) ?
\$element\[\'settings\'\] : \[\];
